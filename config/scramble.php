...
use App\OpenApi\Extensions\ValidationEnvelopeExtension;
...
return [
    ...
    'info' => [
         ...
         */
        'description' => <<<'MD'
Hardware Supply API — customer storefront and staff catalog management.

**Conventions**

- Success payloads are wrapped under a top-level `data` key; list responses use resource collections with pagination meta.
- Every error is returned as `{ "error": { "code", "message", "details" }, "request_id" }` where `code` is a stable machine-readable string (e.g. `VALIDATION_ERROR`, `TWO_FACTOR_REQUIRED`). Field validation failures carry the per-field messages under `error.details.fields`.
- Authenticated endpoints expect a Sanctum personal access token: `Authorization: Bearer <token>`.
- Most endpoints are rate limited; throttled calls answer `429` with a `Retry-After` header.
MD,
    ],

    'ui' => [
        'title' => 'Hardware Supply API',
    ],
    ...
    /*
     * Documentation extensions: exception-to-response mappers that render every
     * error as the ApiExceptionRenderer envelope, plus operation transformers
     * documenting throttle/permission middleware outcomes.
     */
    'extensions' => [
        HttpEnvelopeExtension::class,
        NotFoundEnvelopeExtension::class,
        AuthorizationEnvelopeExtension::class,
        AuthenticationEnvelopeExtension::class,
        ValidationEnvelopeExtension::class,
        TwoFactorRequiredEnvelopeExtension::class,
        SuspendedAccountEnvelopeExtension::class,
        ProductNotPublishableEnvelopeExtension::class,
        CategoryInUseEnvelopeExtension::class,
        InsufficientStockEnvelopeExtension::class,
        NegativeStockEnvelopeExtension::class,
        VariantNotPurchasableEnvelopeExtension::class,
        PriceUnavailableEnvelopeExtension::class,
        CouponEnvelopeExtension::class,
        CheckoutTotalsChangedEnvelopeExtension::class,
        CartEmptyEnvelopeExtension::class,
        PaymentMethodUnavailableEnvelopeExtension::class,
